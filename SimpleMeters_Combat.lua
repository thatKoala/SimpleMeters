...
state.dataDirty = state.dataDirty == true or state.dirty
state.inFight = state.inFight == true
state.awaitingFightEnd = state.awaitingFightEnd == true
state.fightEndAt = state.fightEndAt or 0
state.pendingFinalizeAt = state.pendingFinalizeAt or 0
state.oocConfirmedAt = state.oocConfirmedAt or 0
...
state.renderDirty = state.renderDirty == true or state.uiDirty
...
local function ClearPendingFinalize()
    state.awaitingFightEnd = false
    state.fightEndAt = 0
    state.pendingFinalizeAt = 0
...
function addon:StartFight(now)
    ...
    MarkDataDirty()
    MarkRenderDirty()
    if addon and addon.WakeUITicker then
        addon:WakeUITicker()
    end
end
...
function addon:OnLeaveCombat(now)
    ...
    now = now or GetTime()
    if state.pendingFinalizeAt <= 0 then
        state.pendingFinalizeAt = now + FIGHT_FINALIZE_FAILSAFE
    end
    state.awaitingFightEnd = true
    state.fightEndAt = now + FIGHT_END_DELAY
    state.oocConfirmedAt = now
    MarkRenderDirty()
    if addon and addon.WakeUITicker then
        addon:WakeUITicker()
    end
...
function addon:UpdateFightTimeout(now)
    ...
    if not state.awaitingFightEnd then
        state.awaitingFightEnd = true
        state.fightEndAt = lastDamageTime + FIGHT_END_DELAY
        state.pendingFinalizeAt = now + FIGHT_FINALIZE_FAILSAFE
        state.oocConfirmedAt = 0
        MarkRenderDirty()
    elseif state.pendingFinalizeAt <= 0 then
        state.pendingFinalizeAt = now + FIGHT_FINALIZE_FAILSAFE
    end

    if state.oocConfirmedAt and state.oocConfirmedAt > 0 then
        FinalizeFight(lastDamageTime)
        return
    end

    if state.pendingFinalizeAt > 0 and now >= state.pendingFinalizeAt then
        FinalizeFight(lastDamageTime)
    end
end
