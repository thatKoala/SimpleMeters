...
function addon:BuildModeCaches(now, force, periodicTotalRefresh)
    self.modeCache = self.modeCache or {}

    local maxNeededByMode = self.maxNeededByModeScratch
    if not maxNeededByMode then
        maxNeededByMode = {}
        self.maxNeededByModeScratch = maxNeededByMode
    end
    for key in pairs(maxNeededByMode) do
        maxNeededByMode[key] = nil
    end
    ...
    for panelId, frame in pairs(self.panelFrames) do
        local panel = frame and frame.cfg
        if panel and panel.shown and frame:IsShown() and panel.mode ~= "boss" then
            local rowLimit = self:GetPanelVisibleRowCount(frame)
            local existing = maxNeededByMode[panel.mode] or 0
            if rowLimit > existing then
                maxNeededByMode[panel.mode] = rowLimit
            end
        end
    end

    for mode, limit in pairs(maxNeededByMode) do
        local cache = self.modeCache[mode]
        local shouldRebuild = force or not cache

        if mode == "fight" then
            if self.state and self.state.inFight then
                shouldRebuild = true
            elseif self.state and self.state.dirty then
                shouldRebuild = true
            end
        else
            if mode == "total" and periodicTotalRefresh then
                shouldRebuild = true
            elseif self.state and self.state.dirty and not (self.state and self.state.inFight) then
                shouldRebuild = true
            end
        end

        if cache and cache.limit and limit > cache.limit then
            shouldRebuild = true
        end

        if shouldRebuild then
            local built = BuildTopEntries(self, mode, now, limit)
            built.limit = limit
            self.modeCache[mode] = built
        end
    end
...
function addon:OnUITick(force)
    ...
    local hasVisibleFight = false
    local hasVisibleTotal = false
    local hasVisibleBoss = false
    ...
    if self.panelFrames then
        for _, frame in pairs(self.panelFrames) do
            if frame and frame.cfg and frame.cfg.shown and frame:IsShown() then
                hasVisible = true
                if frame.cfg.mode == "fight" then
                    hasVisibleFight = true
                elseif frame.cfg.mode == "total" then
                    hasVisibleTotal = true
                elseif frame.cfg.mode == "boss" then
                    hasVisibleBoss = true
                end
            end
        end
    ...
    local forceFightTick = hasVisibleFight and self.state.inFight
    local totalPeriodicTick = false
    if self.state.inFight and hasVisibleTotal then
        local totalDamage = tonumber(self.state.totalDamage) or 0
        if totalDamage <= 0 then
            totalPeriodicTick = true
        else
            local nextTotalRefreshAt = tonumber(self.state.nextTotalRefreshAt) or 0
            if now >= nextTotalRefreshAt then
                totalPeriodicTick = true
                self.state.nextTotalRefreshAt = now + 10
            end
        end
    end

    local uiDirty = self.state.uiDirty == true
    local skipDirtyOnlyTotalCombat = (not force)
        and self.state.inFight
        and hasVisibleTotal
        and not hasVisibleFight
        and not hasVisibleBoss
        and self.state.dirty
        and not totalPeriodicTick
        and not uiDirty
    if skipDirtyOnlyTotalCombat then
        return
    end

    if not force and not self.state.dirty and not forceFightTick and not totalPeriodicTick and not uiDirty then
        return
    end

    local modeCaches = self:BuildModeCaches(now, force, totalPeriodicTick)
...
function addon:CreateUI()
    ...
    ticker.tickElapsed = ticker.tickElapsed or 0
    ticker:SetScript("OnUpdate", function(_, elapsed)
        if not addon.state then
            return
        end

        local hasPendingLookups = addon.state.pendingGUIDLookupSet and next(addon.state.pendingGUIDLookupSet) ~= nil
        local hasPendingBoss = addon.state.pendingBossKills and #addon.state.pendingBossKills > 0
        local needsTick = addon.state.inFight or addon.state.awaitingFightEnd or hasPendingLookups or hasPendingBoss

        if needsTick then
            ticker.tickElapsed = ticker.tickElapsed + elapsed
            local runs = 0
            while ticker.tickElapsed >= 0.75 and runs < 2 do
                ticker.tickElapsed = ticker.tickElapsed - 0.75
                addon:OnUITick(false)
                runs = runs + 1
            end
            if ticker.tickElapsed > 1.5 then
                ticker.tickElapsed = 0.75
            end
        else
            ticker.tickElapsed = 0
            if addon.state.dirty then
                addon:OnUITick(false)
            end
        end
    end)
